...
// ── JS-shell / challenge detection (drives the headless-Chrome fallback) ────

/** Markers that say "this page is an anti-bot / JS challenge, not a SERP". */
export const GATE_MARKERS = [
	/enable javascript/i, /please enable javascript/i, /javascript is required/i,
	/captcha/i, /unusual traffic/i, /cloudflare/i, /attention required/i,
	/verify you are human/i, /security check/i, /access denied/i,
	/are you not a robot\?/i, /punish\?x5sec/i, /x5secdata/i,
	/robots?( are)? (not|denied)/i
];

/** Per-engine regex telling "the page actually carries this engine's results". */
const RESULT_MARKER_BY_ENGINE = {
	bing: /<item>|<rss|b_algo/i,
	bingcn: /<item>|<rss/i,
	baidu: /<h3|<div[^>]*class="result/i,
	sogou: /<h3|vrwrap/i,
	so360: /<h3|res-list/i,
	smcn: /<h3|result/i,
	google: /<h3|LC20lb/i,
	duckduckgo: /result__a|result__snippet/i,
	brave: /snippet-title|snippet-description/i,
	mojeek: /<h2/i,
	qwant: /result__/i,
	ecosia: /result__|js-result/i,
	yahoo: /<h3|compText/i,
	yandex: /serp-item/i,
	startpage: /w-gl__result/i,
	searx: /<article/
};

/** True when `html` already contains this engine's result markup. */
export function hasResultMarkers(engine, html) {
	const re = RESULT_MARKER_BY_ENGINE[engine.id];
	return re ? re.test(String(html)) : /<h3|result/i.test(String(html));
}

/**
 * True when the body looks like a JS-shell / anti-bot challenge page (no
 * result markers but a challenge marker) — i.e. plain `fetch` can never parse
 * it, so the headless-Chrome fallback is warranted.
 */
export function looksLikeJsShell(engine, html) {
	return !hasResultMarkers(engine, html) && GATE_MARKERS.some((re) => re.test(String(html)));
}
